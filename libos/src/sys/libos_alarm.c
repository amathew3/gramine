...
static void signal_timerfd_settime(IDTYPE caller, void* arg) {
    __UNUSED(caller);
...
long libos_syscall_timerfd_settime(int fd, int flags, struct __kernel_itimerval* value,
                             struct __kernel_itimerval* ovalue) {
    ...
    //int64_t install_ret = install_async_event(NULL, next_value, &signal_timerfd_settime,
    //                                          (void*)(setup_time + next_value));
    //timerfd_pal_write(PAL_HANDLE handle, uint64_t offset, uint64_t len,
    //                             const void* buffer)
    //if (install_ret < 0) {
    //    spinlock_unlock(&g_real_itimer_lock);
    //    return install_ret;
